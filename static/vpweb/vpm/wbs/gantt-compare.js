try{
	...
	vpGantt.getData=function(projectId,taskId){
		...
		var params = {projectID: projectId, wbsObjId: taskId, relentityId: relentityId, i1type:0, item1Version:0, i2type:1 , item2Version:0};
	...
	vpGantt.rendeIndicater=function(v){
		...
		vpGantt.columns=[{header:"ID",sortable:false,width:45,dataIndex:"Id",menuDisabled:true,renderer:function(v,m,r){return v},locked:true}
		,{xtype:"treecolumn",header:"任务名称",sortable:false,menuDisabled:true,dataIndex:"Name",width:150,field:{allowBlank:false},renderer:function(value,metaData,record,rowIdx,colIdx,store){var objId=31;if(record.get("leaf")){metaData.tdCls="task";objId=33}if(record.get("Duration")==0){metaData.tdCls="milestone";objId=32}if(!record.get("leaf")){metaData.tdCls="abstract-task";objId=34}return'<lable href="#" onclick="vpGantt.openNextLevelWin('+objId+","+vpGantt.getProjectId()+","+record.get("Id")+');">'+value+"</lable>"}}
		,{header:'<table class="head-table"><tr><td>开始日期</td></tr><tr><td>延迟</td></tr></table>',menuDisabled:true,sortable:false,width:75,renderer:function(value,metaData,record,rowIdx,colIdx,store){if(!record.get("StartDate")||!record.get("BaselineStartDate")){return""}else{var num=(record.get("StartDate").getTime()-record.get("BaselineStartDate").getTime())/86400000;num=Math.round(num*100)/100;return(num==0||num<0)?"":"延迟"+num+"天"}}},{header:'<table class="head-table"><tr><td>完成日期</td></tr><tr><td>延迟</td></tr></table>',menuDisabled:true,sortable:false,width:75,renderer:function(value,metaData,record,rowIdx,colIdx,store){if(!record.get("StartDate")||!record.get("BaselineStartDate")){return""}else{var num=(record.get("EndDate").getTime()-record.get("BaselineEndDate").getTime())/86400000;num=Math.round(num*100)/100;return(num==0||num<0)?"":"延迟"+num+"天"}}}
		,{header:"资源",menuDisabled:true,sortable:false,width:120,dataIndex:"ResIds"}];

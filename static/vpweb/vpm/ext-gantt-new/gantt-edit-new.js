try {
	...
	function copyTask(c) {
		...
		if(c.get("StartDate")!=undefined&&c.get("StartDate")!=null){
	...
	vpGantt.getCalendar = function() {
		var data,result=[];
		var params = {projectID: vpGantt.getProjectId(),isWBSList: true};
		vpPostAjax(getVpmPath() + '/vpmwbs/getWorkCalendarItems', params, "POST", function(rst) {
			data = rst.data;
		});
	...
	vpGantt.getData = function(projectId, taskId) {
		...
		var params = {projectID: projectId, relentityId: relentityId, wbsObjId: taskId};
	...
	vpGantt.rendeIndicater = function(v,m,r,ri) {
		...
		var flags='<div style="border:0px solid #000;position:relative;width:16px;text-align:center;left:-8px;"><img  src="../../vpm/images/wbstask/' + l1 + '" title="' + t1 + '" />';
		if(parseInt(r.get('Option2'))==0){
			flags+='<img  src="../../vpm/images/wbstask/icon_cjyes.gif" title="可裁剪" />'
		}else{
			flags+='<img  src="../../vpm/images/wbstask/icon_cjno.gif" title="不可裁剪" />'
		}
		flags+='</div>';
		return flags;
	};
	...
	vpGantt.getAllTasksStr = function() {
		...
		var rootNode = this.getRootNode(),tasks = ['{"deldIds":'+json.toJSON(vg.deldIds)+',"delsIds":'+json.toJSON(vg.delsIds)+',"tasks":['];
	...
	vpGantt.getSingleNodeStr = function(snode) {
		...
		var curRecodeID="";
		var curRecodeDependencyID="";
		for(var w=0,l1=subFs.length;w<l1;w++){
			var cf=subFs[w];
			var cfname=cf.name;
			var cfval=cf.getVal(snode);
			if(cfname=='Id'){
				curRecodeID=cfval;
			}
			if(cfname=='DependencyTask'){
				curRecodeDependencyID=cfval;
			}
			if(curRecodeID==curRecodeDependencyID){
				cfval="";
			}
			
			if(w==0){
				curStr.push( '"'+cfname+'":"' +cfval);
			}else{
				curStr.push( '","'+cfname+'":"' +cfval);
			}
		}
	...
	vpGantt.getNewStartDate = function(parentNode) {
		...
		if (parentNode && ((vpGantt.getTaskId()==null&&!parentNode.parentNode.isRoot())||(vpGantt.getTaskId()!=null&&!parentNode.isRoot()))) {
			if(parentNode.get("StartDate")!=undefined&&parentNode.get("StartDate")!=null){
				if (today.getTime() > parentNode.get("StartDate").getTime()) {
					return new Date(today.getFullYear(), today.getMonth(), today.getDate());
				} else {
					return parentNode.get("StartDate");
				}
			} else {
			    return new Date(today.getFullYear(), today.getMonth(), today.getDate());
			}
	...
	vpGantt.downGrade = function() {
		...
		if (!cur.previousSibling) {
			...
			if(preNode.getMapped()>0){
				alert('当前任务的前一个兄弟节点为镜像任务,不能对其降级!');
				vpGantt.gridMask.hide();
				return;
			}
	...
	vg.deldIds=[-1,-2,-3];
	vg.delsIds=[-1,-2,-3];
	vg.getDelMaps=function(id,n){
		vg[n].push(parseInt(id));
	};
	vpGantt.checkMap=function(p){
		var ids=[];
		vpGantt.accessAllNodes(p,{func:function(n){
			ids.push(n.getId());
		}});
		var r;
		if(ids.length>0){
			var params = {projectId: vpGantt.getProjectId(), relentityId: relentityId, ids:ids.join(',')};
			vpPostAjax(getVpmPath() + '/vpmwbs/loadWBSMap', params, "POST", function(rst) {
				r = rst.data;
			});
		}
		return r;
	...
	vpGantt.delTask = function() {
		var cur = vpGantt.getSelSingleTask();
		if(cur.data.Option2 != '0'){ //不可裁剪
			alert('当前任务为不可裁剪,不允许删除!');
			vpGantt.gridMask.hide();
			return;
		}
		var r = vpGantt.checkMap(cur),jump = false;
		if(r.d||r.s){
			var msg='';
		...
		vpGantt.changeComponent.setOtherChanged(true);
		if(cur.getMapped()==1){
			vg.getDelMaps(cur.getId(),'deldIds');
		}else if(cur.getSrcMapped()==1){
			vg.getDelMaps(cur.getId(),'delsIds');
		}
		if(delIds.indexOf("T_") == -1){
		    delIds += cur.getId() + ",";
		}
		vpGantt.gridMask.hide();
	};
	...
	vpGantt.importProject = function(task, toolbar, pos) {
		...
		if (task) {
			...
			var curTaskId = (toolbar ? vpGantt.getTaskId() : vpGantt.getSelSingleTask().get("WbsId"));
			vpGantt.showModalWin('../../vpm/wbs/import4Project.html?isTemplate=0&projectId=' + projectId + '&relentityId=' + relentityId + '&wbsId=' + curTaskId  + '&pos=' + pos + '&idIsReal=' + toolbar, title, height, width);
		} else vpGantt.showModalWin('../../vpm/wbs/import4Project.html?isTemplate=0&projectId=' + projectId + '&relentityId=' + relentityId + '&pos=' + pos + '&idIsReal=' + toolbar, title, height, width);
	};
	...
	vpGantt.saveAllData = function(quit) {
		if(vpGantt.getTaskId() == null){
			...
			//保存批量根结点WBS任务
			var params = {projectID: vpGantt.getProjectId(), codetype: 'utf8', relentityId: relentityId, delIds: delIds, wbsObjId: vpGantt.getTaskId(), tasks: decodeURI(decodeURI(vpGantt.getRootData()))};
			vpPostAjax(getVpmPath() + '/vpmwbs/saveRootTasks', params, "POST", function(rst) {
				if(rst.success){
					vpGantt.saveDataWithoutRoot(quit);
				}
			});
	...
	vpGantt.saveDataWithoutRoot=function(quit){
		//保存WBS任务
		var params = {projectID: vpGantt.getProjectId(), codetype: 'utf8', relentityId: relentityId, delIds: delIds, wbsObjId: vpGantt.getTaskId() == null ? 0 : vpGantt.getTaskId(), tasks:vpGantt.getAllTasksStr()};
		vpPostAjax(getVpmPath() + '/vpmwbs/saveTasks', params, "POST", function(rst) {		
			if (rst.success) {
				vpGantt.changeComponent.clear();
				//window.location.reload();
				vpGantt.reloadProject();
				if(quit){
					//parent.location.reload();  //刷新父页面数据					
					//dialog.close();       //关闭当前窗口  top.window.close();
					parent.window.appdoclist.closeWBSModal();   //react方法调用
					//parent.window.close();
				} else {
					try{
						//var url = parent.parent.document.getElementById("LayerFrame").contentWindow.document.getElementById("LayerFrame").src;
						//parent.parent.document.getElementById("LayerFrame").contentWindow.document.getElementById("LayerFrame").src = url;
						//parent.openeditWin(vpGantt.getTaskId(), vpGantt.getProjectName(), vpGantt.getProjectId());
					}catch(e){
						//parent.openeditWin(vpGantt.getTaskId(), vpGantt.getProjectName(), vpGantt.getProjectId());
						//var url = parent.document.getElementById("infoIframe").contentWindow.document.getElementById("formIframe").src;
						//parent.document.getElementById("infoIframe").contentWindow.document.getElementById("formIframe").src = url;						
					}
					vpGantt.gridMask.hide();
					vpGantt.refreshData(false,'0','true');      //关闭重新加载
					parent.window.appdoclist.openWBSModal();
				}
				return;
				vpGantt.reload(rst.data);
				vpGantt.changeComponent.clear();
				...
				vpGantt.afterEWIndecator = rst.data.failMsgID;
				vpGantt.gridMask.hide();
				vpGantt.showErrorMsg();
				jQuery('#error-div').html(rst.data.failMsg);
				if (rst.data.items && rst.data.items.length > 0) {
					...
					for (var i = 0, items = rst.data.items; i < items.length; i++) {
	...
	vpGantt.showCalendar=function(e){
		...
		//x = (document.body.clientWidth - width) / 2,
		//y = (document.body.clientHeight - height) / 2,
		src='../../vpm/wbs/pjCalendar.html?projectID='+vpGantt.getProjectId()+'&editable='+(this.changeComponent.isChanged()?0:1);
...
/*		{
	...
	},
	{
		...
		handler: function() {
			jQuery.ajax({
				url: '/project/wbs/ganttLoadAction.do?themethod=setWorkPlanView&withgantt=0',
				type: 'get',
				async: false,
				success: function(res) {
				}
			});
			window.location.href=window.location.href.replace('/ganttEdit-new.jsp','/sim/ganttEdit-new.jsp');
		}
	},
	{
		text: '关键路径',
	...
	vpGantt.mappedTask={};
	vpGantt.srcMappedTask={};
	vpGantt.cacheMappedTask=function(t,type){
		this[type][t.internalId]=1;
	};
	...
	vpGantt.cacheTask=function(id,task){
		...
		if(task.getMapped()>0){
		//if(task.getId()%4==1&&task.childNodes.length<1){
			vpGantt.cacheMappedTask(task,'mappedTask');
		}
		if(task.getSrcMapped()>0){
				vpGantt.cacheMappedTask(task,'srcMappedTask');
		}
	};
	...
	*/
	...
	Ext.onReady(function() {
		...
		/*vpGantt.gantt.lockedGrid.on("itemclick",function(ttt,record,  index,  eOpts ){
			return;
			if(vpGantt.keyPathShow===true){
				vpGantt.toggleKeyPath(true);
				var nView=vpGantt.getNormalView();
				nView.select(record);
			}
		});
		vpGantt.gantt.normalGrid.on("itemclick",function(ttt,record,  index,  eOpts ){
			return;
			if(vpGantt.keyPathShow===true){
				vpGantt.toggleKeyPath(true);
				var nView=vpGantt.getNormalView();
	...
	vpGantt.buildGrid = function() {
		...
		vpGantt.gantt.lockedGrid.getView().on('beforedrop', function(node, data, overModel, dropPosition, dropHandlers) {
		...
		vpGantt.gantt.on('selectionchange',function(o, record, index, eOpts){
			...
			if(res&&res.length>1){
				var it=res[0].rIndex+1,ib=res[res.length-1].rIndex+1;
				ti=vpGantt.copybt1.parentNode.scrollHeight/22;
				var offb=0,offt=0;
				if(ib>=ti){
					offb=44;
				}
				if(it!=1){
					vpGantt.copybt1.style.top=(it*22-44)+'px';
					vpGantt.showCopyBt1();	
				}
				
				
				vpGantt.copybt2.style.top=(ib*22-1+offb)+'px';
				vpGantt.showCopyBt2();
			}else{
				vpGantt.hideCopyBt1();
				vpGantt.hideCopyBt2();
		...
		vpGantt.gantt.lockedGrid.on('beforeitemexpand',function(o, record, index, eOpts){
		...
		vpGantt.gantt.lockedGrid.on('beforeitemcollapse',function(o, record, index, eOpts){
			vpGantt.deselectSels();
			return true;
			if(vpGantt.getSelTaskCount()>1){
				return false;
	...
	vpGantt.reload = function(data) {
		...
		if(vpGantt.getTaskId()!= null){
			...
			tasks=[{KeyTask:false,IndicateLight:"{-1}{-1}{-1}{-1}","Id":"",leaf:false,Name:vpGantt.getProjectName(),expanded:true,children:data.tasks}];
		}
		this.taskStore.proxy.data =tasks;

		// 默认展开
/*		var expandAll = function(tasks) {
			tasks.map((item, value) => {
	...
	vpGantt.markMappedTask=function(){
		var view=vpGantt.gantt.lockedGrid.getView();
		var pre=view.id+'-record-',m=vpGantt.mappedTask,sm=vpGantt.srcMappedTask;
		for(var i in m){
			if(m.hasOwnProperty(i)){
				var d=document.getElementById(pre+i);
				if(d){d.className +=' x-grid-mapped-row'};
			}
		...
		for(var i in sm){
			if(sm.hasOwnProperty(i)){
				var d=document.getElementById(pre+i);
				if(d){d.className +=' x-grid-srcmapped-row'};
			}
	...
	vpGantt.loadData = function() {
		try {
			...
			var data=vpGantt.data;
			vpGantt.reload(data);
	    	vpGantt.scrollToDate(new Date());
	    	vpGantt.genResCls(data);
	    	vpGantt.markMappedTask();
	    	vpGantt.gantt.lockedGrid.getView().on({refresh:vpGantt.markMappedTask});
	    	//vpGantt.moveAGrid();
		} catch(e) {
			alert(e.message);
			vpGantt.gridMask.hide();
	...
	vpGantt.createRowMenu = function() {
		...
		vpGantt.mRowMenu=Ext.create('Ext.menu.Menu',{
			id: 'mRowMenu',
			items: [{
				iconCls: 'arrow2r',
				text: '删除',
	...
	vpGantt.disableDelete = function() {
		vpGantt.disableRowMenuItem(6);
	};
	vpGantt.enableDelete = function() {
		vpGantt.enableRowMenuItem(6);
	};

	vpGantt.handleDeleteButForAlreadyMirror = function() {
		var taskInfo = vpGantt.curSelTask;
		if(taskInfo.raw.hasmirror && 'true' === taskInfo.raw.hasmirror){
			vpGantt.disableDelete();
		}else{
			vpGantt.enableDelete();
		}
	};
	...
	vpGantt.virtualResMg=function(){
		dialog.openQuery('虚拟资源管理', '/project/wbs/virtualResourceAction.do?wbs=wbs&event=getResource&projectID='+vpGantt.getProjectId());
		/*
	...
	vpGantt.moveAGrid=function(){ 
		...
		if((!this.withChrome())&&this.resourceStore.getCount()>4){
			css.setAttribute("href","../../vpm/css/gnt-asgrid-move.css");

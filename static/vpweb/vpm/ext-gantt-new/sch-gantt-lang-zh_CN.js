...
( function() {
	...
	Ext.define("Gnt.column.ComboBox.HDEditor", {
		extend : "Ext.grid.CellEditor",
		alias : "widget.vphdlxditor",
		context:null,
		constructor : function(a) {
			a = a || {};
			var comboxStore=vpGantt.getComboStore(a.colId);
			a.field =  Ext.create('Ext.form.ComboBox', {
			    ...
			    displayField:'name',
			    valueField: 'val',
			    listConfig:{tpl : '<tpl for="."><div style="overflow:hidden;padding-left:{val.length*20/3-15}px;" class="x-boundlist-item ">{name}</div></tpl>'},
	...
	Ext.define("Gnt.column.HdlxCombo", {
		extend : "Ext.grid.column.Column",
		requires : ["Ext.form.ComboBox","Gnt.column.ComboBox.HDEditor" ],
		header : "comboxTest",
		align : "left",
		alias : "widget.vphdlx",
		constructor : function(a) {
			a = a || {};
			a.editor =Ext.create("Gnt.column.ComboBox.HDEditor",{colId:a.dataIndex});
			this.scope = this;
			this.callParent([ a ])
		},
		renderer:function(v){
	...
	Sch.plugin.TreeCellEditing.prototype.startEdit=function(a,f){
		try{
			...
			if(a.get('taskcount')!=null&&a.get('taskcount')!=undefined&&a.get('taskcount')!='0'&&parseInt(a.get('taskcount'))>0){
                return;
			}
			if(vpGantt.mappedTask[a.get('Id')]){
				return;
			}

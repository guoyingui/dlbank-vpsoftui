...
function getVpmPath() {
	return vp.gateway.handleGateWay('{vppm}');
}
...
function vpAjax(url, param, type, callback) {
	...
	$.ajax({
		...
		// contentType: "application/text;charset=utf-8",  // 网络错误0x2efd，与Keep-Alive要么关闭  不能加这个，加了后，参数无法传递 2020/4/13 
		dataType: "json",
	...
	vpPostAjax(obj.vpurl, obj.param, 'GET', function (rst) {
		...
		layui.use('table', function () {
			olisttable.getTable().render({
				...
				data: rst.data,
				limit: ((obj.showpage != false) ? rst.numperpage : 1000),
				page: false,
				initSort: {

...
var HIGHFLOW_COLOR = Color.Green2
...
var LABEL_FONT = {font: "14px Arial", "font-style":"italic", opacity: 1, "fill": LABEL_COLOR};
var LABEL_FONT_SMOOTH = {font: "10px Arial", "font-style":"italic", opacity: 1, "fill": LABEL_COLOR, stroke: LABEL_COLOR, "stroke-width":.4};
var TASK_FONT = {font: "14px Arial", opacity: 1, fill: Color.black};
var TASK_FONT_SMOOTH = {font: "14px Arial", opacity: 1, fill: Color.black, stroke: LABEL_COLOR, "stroke-width":.4};
var POOL_LANE_FONT = {font: "14px Arial", opacity: 1, fill: Color.black};
var EXPANDED_SUBPROCESS_FONT = {font: "14px Arial", opacity: 1, fill: Color.black};
...
// TODO: generate polylineId or do something!!
		...
		*/
		...
		}).hover(function (mouseEvent) {//hover触发
			var instance = this;
			var objectId = instance.data("objectId");
			var object = this.paper.getById(objectId);
			var contextObject = object.data("contextObject");
	...
	*/
	...
	fitTextToWidth: function(original, width) {
		...
		var attr = {font: "14px Arial", opacity: 0};
	...
	*/
	...
	highFinsishActivity: function(activityId){
		var shape = this.g.getById(activityId);
		if (!shape) {
			console.error("Activity " + activityId + " not found");
			return;
		...
		var contextObject = shape.data("contextObject");
		if (contextObject)
			console.log("--1> highLightActivity: ["+contextObject.getProperty("type")+"], activityId: " + contextObject.getId());
		else
			console.log("--1> highLightActivity: ", shape, shape.data("contextObject"));
		
		shape.attr("stroke-width", THICK_TASK_BORDER_STROKE);
		shape.attr("stroke", HIGHFLOW_COLOR);
	},	
	highLightFlow: function(flowId){
		...
		st.attr("stroke", HIGHFLOW_COLOR);
		var withArrowHead = shapeFlow.data("withArrowHead");
		if (withArrowHead)
			st[1].attr("fill", HIGHFLOW_COLOR);

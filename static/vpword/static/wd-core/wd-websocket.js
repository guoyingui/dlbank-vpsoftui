...
(function (window, $) {
    ...
    VE.plugin(function () {
        ...
        //连接websocket地址
        editor.socket = io.connect(config.websocketUrl() + '/docroom', {
            query: {
                token: token,
        ...
        //在线
        editor.socket.on('reconnect', function () {
            heartCheck.reset().start();
            VE.log('token=' + token + '服务器重新连接成功.....')
        })
        ...
        editor.socket.on('online', function (userList) {
            ...
            for (var i = 0, len = userList.length; i < len; i++) {
                editor.headertoolbar.addOnline(userList[i].userId, userList[i].userName);
            }
        })
        //一个页面多开tab 监听离线
        ...
        editor.socket.on('doc lock', function (code, data) {
            ...
            pushResult.tmpselflock = user ? pushResult.lockowner == user.userid : false
                            ...
                            message_dialog('Confirm', {
                                ...
                                confirm: function () {
                                    WD_URL.rest.ajaxMask({
                                        ...
                                        type: 'PUT',
                                        data: {
                                            outlineIds: self.target.data('outlineid')
                                        },
                                        dataType: 'json',
                                        maskinfo: '取消中...'
                                    }, function (json) {
        ...
        var heartCheck = {
            timeout: 480000, // 8分钟发一次心跳，比server端设置的连接时间稍微小一点，在接近断开的情况下以通信的方式去重置连接时间。
            ...
            start: function () {
                this.serverTimeoutObj = setInterval(function () {
                    if (editor.socket.connected) {
                        VE.log("连接状态，发送消息保持连接");
                        editor.socket.emit("ping");

...
(function (window, $) {
    ...
    VE.plugin(function() {
        ...
        VE.CHECK = {
            ...
            //验证大纲及其下级子大纲是否有锁
            checkLockChild: function (outlineId,validateSelf) {
                var def = $.Deferred();
                WD_URL.rest.ajax({
            ...
            checkOutlineChildInfo: function (outlineId) {
                return VE.CHECK.checkOutlineExist(outlineId).then(
                    function (data) {
                        return VE.CHECK.checkLockChild(outlineId,false);
                    });
            },

            //检查大纲是否存在，并检查大纲是否被锁住,自身上传
            checkOutlineChildInfoNoSelf: function (outlineId) {
                return VE.CHECK.checkOutlineExist(outlineId).then(
                    function (data) {
                        return VE.CHECK.checkLockChild(outlineId,true);
                    });
            }
        }
    })

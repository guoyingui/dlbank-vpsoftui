...
(function () {
    ...
    VE.pluginPageRender(function ($editorContent) {
        ...
        VE.LOCK.canOprLock = editor.canOprLock;
    ...
    VE.plugin(function () {
        VE.LOCK = {
            ...
            initLockTag:function ($outlines) {
                WD_URL.rest.ajax({
                    ...
                    if(map){
                        ...
                        if(!$.isEmptyObject(map["outlineDownloadLock"])){
                        ...
                        if(!$.isEmptyObject(map["outlineLock"])){
            ...
            addDocOtherLockTag:function($lockInfo){
                var title =  $(".vrm-content-title");
                if(title.find(".vrm-doc-lock").length  <= 0 ){
                    if(VE.LOCK.canOprLock){
                        $(".vrm-content-title").append('<button type="button" class="btn btn-danger  btn-xs pr vrm-tag vrm-doc-lock"><span class="times vrm-doc-lock-del" >x</span><i class="fa fa-lock  vrm-doc-lock">已锁</i></button>' );
                        $(".vrm-doc-lock-del").on('click', function (e) {
                            e.stopPropagation();
                            message_dialog('Confirm', {
                                title : '提示',
            ...
            removeDocOutlineLock:function (outlineId, obj, callback) {
                return WD_URL.rest.ajax({
                    ...
                    if(obj != null){
                        ...
                        VE.LOCK.removeDocOutlineTag(outlineId,"_otherlock");
                    }
